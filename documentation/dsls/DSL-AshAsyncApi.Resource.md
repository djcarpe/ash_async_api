...
| [`segment_naming`](#async_api-segment_naming){: #async_api-segment_naming } | `:snake \| :camel \| :pascal \| (any -> any) \| mfa` |  | How the `:_domain` and `:_resource` address segments render for this resource's channels, overriding the domain's `segment_naming` and any application config. `:snake` snake-cases the type, `:camel` lower-camelizes it, `:pascal` upper-camelizes it, and a one-argument function or `{module, function, args}` receives the domain or resource module and returns the segment string. When unset, the domain's setting (or the `:ash_async_api_segment_naming` application config, then `:snake`) applies. |
